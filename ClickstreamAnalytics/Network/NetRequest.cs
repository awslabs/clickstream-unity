...
namespace ClickstreamAnalytics.Network
{
    internal static class NetRequest
    {
        ...
        {
            var encodeContent = ClickstreamCompress.Gzip(eventJson);
            var bodyRaw = Encoding.UTF8.GetBytes(encodeContent);
            ...
            var url =
                $"{endpoint}?platform=Unity&appId={appId}&event_bundle_sequence_id={1}&upload_timestamp={timestamp}";
